...
\begin{figure}[htbp]
    ...
    \caption{Et eksempel på en spilleplade opdelt i $5 \times 5$ grid, samt visualisering af både HSV-features (terrænklassificering) og HSV-gray/Canny-features (krone-detektion) fra et felt med krone.}
    \label{fig:grid_extraction}
\end{figure}

Figur~\ref{fig:grid_extraction} viser, at systemet bruger forskellige feature-repræsentationer til forskellige delopgaver: HSV-histogrammer til terrænklassificering og en HSV-baseret gråtoneprojektion med Canny-kanter til krone-detektion. Figuren er bevidst taget fra et felt, hvor der faktisk findes en krone, så sammenhængen mellem feature-repræsentation og objektlokalisering fremstår tydeligt.
...
I Figur~\ref{fig:templatematching} vises de endelige detektioner på pladen efter hele krone-pipelinen. Da den aktuelle model finder kroner stabilt på dette eksempel, giver en rå-vs-verificeret sammenligning begrænset ekstra information i netop dette tilfælde.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/template_reference.png}
    \caption{Visualisering af en anvendt krone-template: original template, HSV-gray repræsentation og Canny-kanter. Figuren viser de billedrepræsentationer, som matcher- og verifikationsleddet baserer sig på.}
    \label{fig:template_reference}
\end{figure}

Figur~\ref{fig:template_reference} forklarer den tekniske mekanik i kronedetektionen og supplerer det overordnede detektionsresultat i Figur~\ref{fig:templatematching}. Kombinationen giver både et output-orienteret og et metode-orienteret billede af samme pipeline.
